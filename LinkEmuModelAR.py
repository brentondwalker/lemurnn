...
class LinkEmuModelAR(LinkEmuModel):
    ...
    def export_torchscript(self, filename, state_dict=None):
        ...
        # CHANGED: Add None (for ar_x) and 0.0 (for TF ratio).
        # strict=False allows the tracer to ignore the python dictionary/list packing inside your slow path
        ###traced_model = torch.jit.trace(model, (dummy_x, dummy_hidden))
        #traced_model = torch.jit.trace(model, (dummy_x, dummy_hidden, None, 0.0), strict=False)
        traced_model = torch.jit.trace(model, (dummy_x, dummy_hidden), strict=False)
    ...
    def export_onnx(self, filename, state_dict=None):
        ...
        x = torch.randn(BATCH_SIZE, SEQ_LEN, INPUT_SIZE)
        hidden = self.new_hidden_tensor(BATCH_SIZE)
        # CHANGED: Pad the dummy inputs tuple to match the new forward signature
        dummy_inputs = (x, hidden, None, 0.0)
